...
import quick3d.tools.ObjectPicker;
...
public class GraphMonitor3D implements Graph3D {
    ...
    private ArrayList<Component> nodes;
    ...
    public Canvas createPanel() {
        ...
        op = new ObjectPicker<>(new ObjectPicker.Selectable<Object>() {
            @Override
            public Object select(int index) {
                synchronized (circuit) {
                    ...
                    for (Connection c : circuit.getConnections()) {
                        if (c.getID() == index - 1000) {
                            System.out.println(c.getSubComponent());
                            return c;
                        }
                    }
                }
    ...
    public synchronized void draw(GL2 gl, GLUT glut, boolean colorPicking) {
        ...
        edges.clear();
//        synchronized (circuit) {
        nodes.addAll(circuit.getComponents());
        edges.addAll(circuit.getConnections());
//        }
        gl.glPushMatrix();
        ...
        if (colorPicking) {
            for (Component n : nodes) {
                p = n.getPos();
                gl.glPushMatrix();
                gl.glTranslatef((float) p.getX(), (float) p.getY(), (float) p.getZ());
                float[] c = op.getColor(n.getID());
                gl.glColor3f(c[0], c[1], c[2]);
                glut.glutSolidSphere(2f, 6, 6);
                gl.glPopMatrix();
            }
            ...
            for (Connection e : edges) {
                ...
                p2 = e.getB().getPos();
                gl.glLineWidth(5f);
                float[] c = op.getColor(e.getID() + 1000);
                gl.glColor3f(c[0], c[1], c[2]);
...
//                                drawCube(gl, (float) p.getX(), (float) p.getY(), (float) p.getZ(), 1f, colorPicking ? i : n.hashCode());
                ...
                drawCube(gl, (float) p.getX(), (float) p.getY(), (float) p.getZ(), .3f, n.hashCode());
            }
        }
...
//                                        System.exit(0);
        ...
        //draw box
        {
            ...
            for (Component n : new java.util.ArrayList<>(circuit.getComponents())) {
                if (n.isFixed()) {
                    p = n.getPos();
                    if (p.getX() < minX) {
                        minX = p.getX();
            ...
            double k = 16;//honeycomb.getShortestDistance();
